...
public interface RpcClient extends Closeable {
   ...
   * own behavior, they may retry on the same server. This can be a feature, for example at startup.
   * In any case, they're likely to get connection refused (if the process died) or no route to
   * host: i.e. their next retries should be faster and with a safe exception.
